...
def run_comprehensive_test():
    ...
    performance_prompts = [
        "Расскажи в 3-4 предложениях о преимуществах искусственного интеллекта",
        "Напиши короткое стихотворение (4 строки) о природе",
        "Объясни, что такое квантовые вычисления, простыми словами",
        "Перечисли 5 основных принципов здорового образа жизни",
    ...
    quality_test_cases = [
        {
            "prompt": "Какие планеты входят в Солнечную систему?",
            "expected_keywords": ["Меркурий", "Венера", "Земля", "Марс", "Юпитер"],
            "must_not_contain": ["Плутон"]  # Плутон теперь не считается планетой
        },
        {
            "prompt": "Напиши рецепт омлета",
            "expected_keywords": ["яйца", "соль", "сковорода", "масло"],
            "must_not_contain": ["сахар", "шоколад"]  # Необычные ингредиенты для омлета
        },
        {
            "prompt": "Что такое Python?",
            "expected_keywords": ["язык", "программирования", "интерпретируемый"],
            "must_not_contain": []
        }
    ]
...
if __name__ == "__main__":
    ...
    result = tester.test_single_request(
        prompt="Объясни, почему небо голубое, в двух предложениях",
        model="gemma3:270m"
    )
